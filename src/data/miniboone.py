...
class Miniboone(DataModule):
    ...
    def load_data(self):
        ...
        features = [c for c in data.columns if not c.startswith("signal")]
        x = data[features]
        y = pd.factorize(data["signal"])[0]
...
data_modules.register_builder("miniboone", Miniboone)
